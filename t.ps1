...
try {
    ...
    Write-Host "Contents of outfile.json:"
...
Write-Host "Parsed Claude response:"
try {
    Get-Content $responseFile | ConvertFrom-Json | Format-List
} catch {
    Write-Host "Failed to parse response. Raw output:"
